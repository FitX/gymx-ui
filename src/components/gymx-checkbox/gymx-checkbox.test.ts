...
describe('GymxCheckbox', () => {
  ...
  describe('Boolean/Single Value Mode', () => {
    ...
    it('emits update:modelValue when clicked (boolean)', async () => {
      const modelValue = ref(false)
      const updateSpy = vi.fn((val: boolean) => { modelValue.value = val })
    ...
    it('uses custom value for single checkbox', async () => {
      ...
      const updateSpy = vi.fn((val: string | boolean) => { modelValue.value = val })
  ...
  describe('Array/Multiple Value Mode', () => {
    it('shows as unchecked when array is empty', () => {
      ...
      const wrapper = mount(GymxCheckbox, {
        props: {
          modelValue: modelValue.value,
          // @ts-expect-error corrupt val test
          'onUpdate:modelValue': (val: unknown) => { modelValue.value = val }
        }
      })
    ...
    it('adds value to array when checked', async () => {
      ...
      const updateSpy = vi.fn((val: string[]) => { modelValue.value = val })
    ...
    it('handles numeric values in arrays', async () => {
      ...
      const updateSpy = vi.fn((val: number[]) => { modelValue.value = val })
    ...
    it('does not add duplicate values to array', async () => {
      ...
      // Checkbox ist bereits checked (da option2 im array ist), sollte entfernt werden
      const input = wrapper.find('input')
      input.element.checked = false // Simulating unchecking
      await input.trigger('change')
  ...
  describe('Props and Attributes', () => {
    ...
    it('uses effectiveValue correctly for boolean without explicit value', () => {
      ...
      // Der effective value sollte true sein (default für boolean)
    ...
    it('uses explicit value when provided', () => {
      ...
      const wrapper = mount(GymxCheckbox, {
        props: {
          ...
          'onUpdate:modelValue': (val: boolean | string) => { modelValue.value = val }
        }
      })
    ...
    it('handles mixed string/number values in arrays', () => {
      ...
      const wrapper = mount(GymxCheckbox, {
        props: {
          modelValue: modelValue.value,
          value: 42,
          'onUpdate:modelValue': (val: (string | number)[]) => { modelValue.value = val }
        }
      })
  ...
  describe('Accessibility', () => {
    it('generates unique IDs when none provided', () => {
      ...
      const wrapper1 = mount(GymxCheckbox, {
        props: {
          modelValue: modelValue1.value,
          'onUpdate:modelValue': (val: boolean) => { modelValue1.value = val }
        }
      })

      const wrapper2 = mount(GymxCheckbox, {
        props: {
          modelValue: modelValue2.value,
          'onUpdate:modelValue': (val: boolean) => { modelValue2.value = val }
        }
      })
